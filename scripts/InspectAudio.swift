...
@main struct InspectAudio {
    static func main() {
        do { try inspect() } catch {
            FileHandle.standardError.write(
                Data("Conch diagnostic: \(error.localizedDescription)\n".utf8))
            exit(1)
        }
    }
    static func inspect() throws {
        ...
        for scope in [kAudioObjectPropertyScopeInput, kAudioObjectPropertyScopeOutput] {
            for stream in try HAL.objects(output, kAudioDevicePropertyStreams, scope: scope) {
                ...
                print(
                    "Stream", stream, "scope", scope, "rate", f.mSampleRate, "channels",
                    f.mChannelsPerFrame, "bits", f.mBitsPerChannel, "flags", f.mFormatFlags)
            }
        }
        ...
        for object in objects {
            ...
            {
                print(
                    "Resolved:", c.owner.localizedName ?? "Unknown", "PID", c.pid, "object",
                    c.object, "active", c.active, "outputs", c.devices.sorted())
            }
        }
    }
}
