...
private final class VolumeBackend: OutputVolumeBackend {
    ...
    func scalar(_ device: AudioObjectID, _ selector: AudioObjectPropertySelector) -> Float32? { values[selector] }
    func muted(_ device: AudioObjectID) -> Bool { isMuted }
    func writable(_ device: AudioObjectID, _ selector: AudioObjectPropertySelector) -> Bool { writableKeys.contains(selector) }
    func writeScalar(_ device: AudioObjectID, _ selector: AudioObjectPropertySelector, _ value: Float32) throws {
        if failWrites { throw AudioFailure(message: "Device disconnected") }
        values[selector] = value; writes += 1
    }
    func writeMute(_ device: AudioObjectID, _ value: Bool) throws { isMuted = value }
    func observe(_ device: AudioObjectID, _ selector: AudioObjectPropertySelector, _ changed: @escaping () -> Void) throws -> AnyObject { callbacks[selector] = changed; return ObservationToken() }
}
...
@Test @MainActor func masterVolumePreferenceAndUnmute() {
    let backend = VolumeBackend(), preferred = OutputVolumeModel.selectors[0], fallback = OutputVolumeModel.selectors[1]
    backend.values = [preferred: 0.4, fallback: 0.3]; backend.writableKeys = [preferred, fallback, kAudioDevicePropertyMute]
    backend.isMuted = true
    let model = OutputVolumeModel(backend: backend); model.start()
...
@Test @MainActor func scalarFallbackAndFixedOutput() {
    let backend = VolumeBackend(), fallback = OutputVolumeModel.selectors[1]
    backend.values = [fallback: 0.6]; backend.writableKeys = [fallback]
    let model = OutputVolumeModel(backend: backend); model.start(); model.setVolume(0.2)
    #expect(backend.values[fallback] == 0.2)
    backend.writableKeys = []; model.refreshProperties(); model.setVolume(0.8)
    #expect(!model.canAdjust && backend.writes == 1 && backend.values[fallback] == 0.2)
    backend.values = [:]; model.refreshProperties()
    #expect(model.volume == nil && !model.canAdjust)
}
@Test @MainActor func externalChangesAndDeviceSwitchAreObserved() async {
    let backend = VolumeBackend(), key = OutputVolumeModel.selectors[0]
    backend.values[key] = 0.2; backend.writableKeys = [key]
    let model = OutputVolumeModel(backend: backend); model.start()
    backend.values[key] = 0.8; backend.isMuted = true; backend.callbacks[key]?()
    await Task.yield()
    #expect(abs(model.volume! - 0.8) < 0.00001 && model.isMuted)
    backend.device = 51; backend.values = [:]
...
@Test @MainActor func failedHardwareWriteDoesNotPretendSuccess() {
    let backend = VolumeBackend(), key = OutputVolumeModel.selectors[0]
    backend.values[key] = 0.3; backend.writableKeys = [key]; backend.failWrites = true
    let model = OutputVolumeModel(backend: backend); model.start(); model.setVolume(0.9)
