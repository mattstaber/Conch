...
@Test func gainIsBounded() {
    ...
    state.setVolume(-2); #expect(state.volume == 0)
    state.setVolume(3); #expect(state.volume == 1)
    state.setVolume(.nan); #expect(state.effectiveGain == 0)
    #expect(VolumeState(volume: .infinity).effectiveGain == 0)
}
@Test func helperGroupingUsesPathBoundary() {
    let paths = ["/Applications/Browser.app", "/Applications/Browser.app/Contents/Helpers/Helper.app"]
    #expect(Ownership.ownerPath(executable: "/Applications/Browser.app/Contents/MacOS/Browser", runningAppPaths: paths) == paths[0])
    #expect(Ownership.ownerPath(executable: "/Applications/Browser.app/Contents/Helpers/Helper.app/Contents/MacOS/Helper", runningAppPaths: paths) == paths[1])
    #expect(Ownership.ownerPath(executable: "/Applications/Browser.app.fake/evil", runningAppPaths: paths) == nil)
...
@Test func meterAttacksAndDecaysWithoutFabrication() {
    ...
    meter.update(peak: 0); #expect(meter.level == 0)
    meter.update(peak: 1); #expect(meter.level == 1)
    meter.update(peak: 0); #expect(abs(meter.level - 0.78) < 0.0001)
    for _ in 0..<50 { meter.update(peak: 0) }
    #expect(meter.level == 0)
    meter.update(peak: .nan); #expect(meter.level == 0)
}
@Test func formatValidationRejectsUnsafeLayouts() throws {
    ...
    format.mBytesPerFrame = 8; format.mSampleRate = 48000
    try HAL.validate(format)
    format.mChannelsPerFrame = 6
    #expect(throws: AudioFailure.self) { try HAL.validate(format) }
    format.mChannelsPerFrame = 2; format.mFormatFlags = kAudioFormatFlagIsSignedInteger
    #expect(throws: AudioFailure.self) { try HAL.validate(format) }
}
import CoreAudio
